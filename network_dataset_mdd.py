...
class Task3Data(data.Dataset):
    ...
    def __init__(self, mask_way='mask',mask_len=10, time_len=30,shuffle_seed=42,is_train = True, is_test = False):
        ...
        self.root = "/userhome/timeinvariant/clean_mdd_timeseries/"
        ...
        self.df = pd.read_csv("/userhome/timeinvariant/mdd_clean.csv")
        ...
        sites = lbls #np.array(self.df['site'])
...
class Task3Baseline(Task3Data):
    def __init__(self, shuffle_seed=42,is_train = True, is_test = False):
        ...
        self.root = "/data5/yang/large/clean_mdd_timeseries/"

        self.df = pd.read_csv("/data5/yang/large/mdd_clean.csv")
        self.df['dx'] = self.df['dx'].astype(int)
    ...
    def __getitem__(self,index):
        name = self.imgs[index]
        lbl = self.lbls[index]
        img = np.load(os.path.join(self.root, f"{self.template}_{name}.npy".replace("adni","sub")))
        slices = [img.T]
        correlation_matrix = self.correlation_measure.fit_transform(slices).mean(0)
        onehot_lbl = np.zeros((2))
        onehot_lbl[lbl] = 1
