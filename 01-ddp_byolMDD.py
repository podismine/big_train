...
import yaml
from network_dataset import Task1DataMDD
...
import argparse
from byol_trainerMDD import BYOLTrainer
import utils
...
def main():
    ...
    train_dataset = Task1DataMDD(root = data_path,mask_way=mask_way,mask_len=mask_len, time_len=time_len)
    if FLAGS.use_ddp is True:
    ...
    model_checkpoints_folder = os.path.join(root, config['saving']['checkpoint_dir'])

    log_dir = os.path.join(config['saving']['log_dir'])
