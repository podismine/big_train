...
from modeling_pretrain import BNTFMDD, MLPHead
...
class BYOLTrainer(nn.Module):
    def __init__(self,depth,heads,m,feature_dim,dim_feedforward,mm,clf_mask=10, mse_mask=5):
        ...
        self.m = m
        self.model_mode = mm
        self.online_network = BNTFMDD(feature_dim,depth,heads,dim_feedforward)
        self.target_network = BNTFMDD(feature_dim,depth,heads,dim_feedforward)
    ...
    def check_values(self):
        if self.model_mode not in ["byol","byol+clf","byol+mse","byol+clf+mse"]:
