from network_dataset_mdd import Task3Data
import torch
import sys
import yaml
import numpy as np
...
from modeling_pretrain import BNTFMDD
...
# config_path = "./configs/params2/abide_tiny.yaml"
# config_path = "./configs/params2/abide_small.yaml"
config_path = args.config_path #"./configs/params2/abide_medium.yaml"
# config_path = "./configs/params/abide_large.yaml"
# config_path = "./configs/params/abide_huge.yaml"

# config_path = "./configs/best_config.yaml"
# config_path = "./configs/best_config2.yaml"

# config_path = "./configs/mask_type/large_select.yaml"
# config_path = "./configs/mask_type/mini_mask.yaml"

# config_path = "./configs/mask_ratio/mse_20.yaml"

config = yaml.load(open(config_path, "r"), Loader=yaml.FullLoader)

# train_dataset = Task3DataFT(shuffle_seed,is_train=True,is_test=False)
# val_dataset = Task3DataFT(shuffle_seed,is_train=False,is_test=False)
# test_dataset = Task3DataFT(shuffle_seed,is_train=False,is_test=True)
mask_way=config['data']['mask_way']
mask_len=int(config['data']['time_mask'])
time_len=int(config['data']['time_len'])
train_dataset = Task3Data(mask_way,mask_len,time_len,shuffle_seed=shuffle_seed,is_train=True,is_test=False)
val_dataset = Task3Data(mask_way,mask_len,time_len,shuffle_seed=shuffle_seed,is_train=False,is_test=False)
test_dataset = Task3Data(mask_way,mask_len,time_len,shuffle_seed=shuffle_seed,is_train=False,is_test=True)
...
encoder = BNTFMDD(feature_size,depth,heads,dim_feedforward).to(device)

load_params = torch.load(os.path.join("/userhome/timeinvariant/TimeInvariant/",config['saving']['checkpoint_dir'],'best_model.pth'),
...
print(res_string)
res_path = config_path.replace("/","_")
with open(f"/userhome/timeinvariant/TimeInvariant/mdd_final_{res_path}.txt", 'a') as f:
